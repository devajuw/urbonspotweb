 ...
 */
function calculateStandardFare(
  hours,
  vehicleType = "4W_Small",
  durationType = "hourly"
) {
...
function calculateTotalFare(
  ...
  const scheduledEnd = new Date(
    startTime.getTime() + bookedHours * 60 * 60 * 1000
  );
  ...
  return {
    ...
    totalAmount:
      (typeof standardFare === "object" ? standardFare.max : standardFare) +
      overtime.charges,
    bookingDetails: {
      startTime: startTime,
  ...
  if (vehicleType && durationType) {
    ...
    if (document.getElementById("fare-estimate")) {
      document.getElementById(
        "fare-estimate"
      ).textContent = `₹${fareInfo.min} - ₹${fareInfo.max}`;
    }
